...
[CustomEditor(typeof(MyMonoBehaviourAdapter.Adaptor), true)]
public class MonoBehaviourAdapterEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        MyMonoBehaviourAdapter.Adaptor clr = target as MyMonoBehaviourAdapter.Adaptor;
        ...
        if (instance != null)
        {
            ...
            foreach (var i in instance.Type.FieldMapping)
            {
                ...
                if (cType.IsPrimitive) //如果是基础类型
                {
                    ...
                    {
                        Debug.LogError($"无法序列化{name}，{e.Message}");
                        EditorGUILayout.LabelField(name, instance[i.Value].ToString());
                    }
                }
                else
                {
                    ...
                    {
                        if (obj == null && cType == typeof(MyMonoBehaviourAdapter.Adaptor))
                        {
                    ...
                    else
                    {
                        ...
                        //}
                        //else
                        EditorGUILayout.LabelField(name, "(n/a)");
                    }
                }
            }
        }
    ...
    private object ConvertSimpleType(object value, Type destinationType)
    {
        ...
        {
            Debug.LogError("无法转换成类型：'" + value.ToString() + "' ==> " + destinationType);
        }
        try
        {
        ...
        {
            Debug.LogError("类型转换出错：'" + value.ToString() + "' ==> " + destinationType + "\n" + e.Message);
